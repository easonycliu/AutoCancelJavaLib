...
public class Settings {
	private final static Map<String, Object> settings = Settings.getAppSettings();
	...
	private static Map<String, Object> getAppSettings() {
		...
		if (app == null) {
			...
			return Map.ofEntries(Map.entry("path_to_logs", "/tmp/autocancel"),
			...
			return Map.ofEntries(Map.entry("path_to_logs", "/tmp/autocancel"),
				...
				Map.entry("default_policy", Settings.getFromJVMOrDefault("default.policy", "base_policy")),
				Map.entry("predict_progress", Settings.getFromJVMOrDefault("predict.progress", "false")),
				Map.entry("monitor_physical_resources", Map.of("CPU", "JVM", "MEMORY", "JVM")),
				Map.entry("monitor_actions", Set.of("/query")));
		} else {
			return Map.ofEntries();
		}
	}
